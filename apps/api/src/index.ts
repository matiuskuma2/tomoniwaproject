...
import { reverseAvailabilityApiRoutes, reverseAvailabilityPublicRoutes } from './routes/reverseAvailability';
import { raOAuthPublicRoutes, raOAuthCallbackRoutes } from './routes/raOAuth';
...
// Reverse Availability Routes (PR-B6 - Public, ご都合伺いゲストページ)
app.route('/ra', reverseAvailabilityPublicRoutes);

// PR-B6 Phase 2: Guest OAuth Routes (ゲストOAuth + FreeBusy自動取得)
app.route('/ra', raOAuthPublicRoutes);          // /ra/:token/oauth/start, /ra/:token/oauth/skip
app.route('/api/ra-oauth', raOAuthCallbackRoutes); // /api/ra-oauth/callback (固定redirect_uri)
...
// 逆アベイラビリティ API (PR-B6: ご都合伺い)
// POST /api/reverse-availability/prepare, /:id/finalize
app.use('/api/reverse-availability', requireAuth);
app.use('/api/reverse-availability/*', requireAuth);
app.route('/api/reverse-availability', reverseAvailabilityApiRoutes);
...
// Internal Scheduling API (Phase R1 - workmate 1:1 scheduling)
app.use('/api/scheduling/internal', requireAuth);
app.use('/api/scheduling/internal/*', requireAuth);
app.route('/api/scheduling/internal', schedulingInternalRoutes);

// Pools API (G2-A - Pool Booking / N対1)
app.use('/api/pools', requireAuth);
app.use('/api/pools/*', requireAuth);
app.route('/api/pools', poolsRoutes);

// People API (People Hub - SSOT統合UI)
