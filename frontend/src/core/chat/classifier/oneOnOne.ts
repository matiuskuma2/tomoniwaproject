...
// 相手の名前パターン（「○○さん」「○○くん」「○○氏」「○○様」「○○と」「○○に」）
// PR-INTENT-1: preference.ts の PERSON_PATTERN と同期して「くん」「氏」「様」をサポート
const PERSON_PATTERNS = [
  /(.+?)さんと/,
  /(.+?)さんに/,  // 「佐藤さんに候補出して」
  /(.+?)くんと/,  // PR-INTENT-1: 「大島くんと予定調整」
  /(.+?)くんに/,  // 「大島くんに候補出して」
  /(.+?)氏と/,    // PR-INTENT-1: 「佐藤氏と会議」
  /(.+?)氏に/,    // 「佐藤氏に候補出して」
  /(.+?)様と/,    // PR-INTENT-1: 「田中様と面談」
  /(.+?)様に/,    // 「田中様に候補出して」
  /(.+?)と(?:の|打ち合わせ|予定|ミーティング|会議|面談|相談)/,  // PR-INTENT-1: 面談/相談も追加
];
...
const RELATIVE_DATE_PATTERNS: Array<{ pattern: RegExp; resolver: () => Date }> = [
  ...
  // PR-UX-12: 「来週も木曜」「来週の火曜」「来週水曜」全てをカバー
  { pattern: /来週[のも]?(月曜|月)/, resolver: () => getNextWeekday(1) },
  { pattern: /来週[のも]?(火曜|火)/, resolver: () => getNextWeekday(2) },
  { pattern: /来週[のも]?(水曜|水)/, resolver: () => getNextWeekday(3) },
  { pattern: /来週[のも]?(木曜|木)/, resolver: () => getNextWeekday(4) },
  { pattern: /来週[のも]?(金曜|金)/, resolver: () => getNextWeekday(5) },
  { pattern: /来週[のも]?(土曜|土)/, resolver: () => getNextWeekday(6) },
  { pattern: /来週[のも]?(日曜|日)/, resolver: () => getNextWeekday(0) },
  { pattern: /今週[のも]?(月曜|月)/, resolver: () => getThisWeekday(1) },
  { pattern: /今週[のも]?(火曜|火)/, resolver: () => getThisWeekday(2) },
  { pattern: /今週[のも]?(水曜|水)/, resolver: () => getThisWeekday(3) },
  { pattern: /今週[のも]?(木曜|木)/, resolver: () => getThisWeekday(4) },
  { pattern: /今週[のも]?(金曜|金)/, resolver: () => getThisWeekday(5) },
];
...
// BUG-1: 「大島くんと調整したい」→ 「予定調整」にはマッチしないが
//   「調整したい」「調整して」にマッチさせる必要がある
//   ただし「調整」単体は誤爆リスクが高いため、person + 「調整」の組み合わせで判定
const TRIGGER_WORDS = [
  '予定調整',
  '日程調整',
  'スケジュール調整',
