...
| ID | 問題 | 影響 | 推奨対応 | 見積もり |
|----|------|------|----------|----------|
| TD-001 | /settings 導線なし | タイムゾーン設定不可 | ヘッダーにメニュー追加 | 30分 |
| TD-002 | apiExecutor.ts 2732行 | 保守困難 | 機能別ファイル分割 | 2日 |
| TD-003 | intentClassifier.ts 763行 | 保守困難 | インテント別ファイル分割 | 1日 |
| TD-004 | ChatLayout.tsx 563行 | 状態管理複雑 | カスタムフック抽出 | 1日 |

### 5.2 🟡 Medium（計画的対応）

| ID | 問題 | 影響 | 推奨対応 | 見積もり |
|----|------|------|----------|----------|
| TD-005 | ChatPane props 15個 | 可読性低下 | Context API 導入 | 2日 |
| TD-006 | ThreadDetailPage 491行 | 保守困難 | コンポーネント分割 | 1日 |
| TD-007 | 孤立ページ多数 | UX 低下 | 統一ナビゲーション | 1日 |
| TD-008 | toLocaleString 直書き残存可能性 | TZ バグ | datetime.ts 統一 | 2時間 |
