...
### Phase Next-3 実装状況（2025-12-30）

#### **Day2: API 実装（2025-12-30 実装完了）**
- ✅ `GET /api/calendar/today` 実装完了（events.list）
- ✅ `GET /api/calendar/week` 実装完了（events.list）
- ✅ JST 境界処理実装（getTodayBounds, getWeekBounds）
- ✅ API 契約安定（range, timezone, events, warning）

#### **Day3: FreeBusy API 実装（2025-12-30 実装完了）**
- ✅ `GET /api/calendar/freebusy?range=today|week` 実装完了
- ✅ API 契約安定（range, timezone, busy, warning）
- ⚠️ **権限不足:** `google_calendar_permission_missing` により `busy` 実データは未取得
- 📝 **UI 対応:** `warning` がある場合は黄色い注意表示

#### **Day4: UI 実装（2025-12-30 実装完了）**
- ✅ Calendar API クライアント実装（`frontend/src/core/api/calendar.ts`）
- ✅ P1 Intents 実装（`schedule.today`, `schedule.week`, `schedule.freebusy`）
- ✅ Calendar Cards 実装（`CalendarTodayCard`, `CalendarWeekCard`, `FreeBusyCard`）
- ✅ ChatLayout state 管理実装（`calendarData`）
- ✅ データフロー完成（User Input → Intent → API → Cards）
- ✅ Warning 表示実装（権限不足時に UI を壊さない）

#### **動作確認済み（2025-12-30 スクリーンショット検証）**
- ✅ `schedule.today`: 「今日の予定は？」→ CalendarTodayCard 表示確認
- ✅ `schedule.freebusy`: 「今日の空き時間は？」→ FreeBusyCard + Warning 表示確認
- ✅ Thread Cards との併存動作確認
- ✅ Warning の UI 安全性確認（`google_calendar_permission_missing` で UI が壊れない）

#### **実装完了・動作確認未実施**
- ⏳ `schedule.week`: 実装完了、動作確認は未実施（スクリーンショット証跡なし）

#### **観察された未確定事象**
- ⏳ 同名イベント重複表示: 「日忘礼経」が2件表示される事象を観察
  - 原因候補: Google Calendar 側データ / recurring イベント処理 / singleEvents 処理 / all-day イベント処理
  - 状態: 原因未確定（現時点で不具合かどうか判断できず）
