# Tomoniwao - ソースコードマップ

**最終更新**: 2026-01-17  
**コミット**: 27c3363

---

## 📊 ファイル統計

| カテゴリ | ファイル数 | 総行数 |
|---------|----------|--------|
| Backend Routes | 27 | ~250,000 |
| Frontend Core | 25+ | ~100,000 |
| Migrations | 62 | ~15,000 |
| Docs | 120+ | ~50,000 |

---

## 🗂️ ディレクトリ構造

```
tomoniwaproject/
├── apps/
│   └── api/                          # Backend (Cloudflare Workers)
│       ├── src/
│       │   ├── index.ts              # エントリポイント
│       │   ├── routes/               # APIルート (27ファイル)
│       │   ├── services/             # ビジネスロジック
│       │   ├── repositories/         # データアクセス
│       │   ├── middleware/           # 認証・CORS等
│       │   └── utils/                # ユーティリティ
│       ├── wrangler.toml             # Cloudflare設定
│       └── package.json
│
├── frontend/                         # Frontend (Cloudflare Pages)
│   ├── src/
│   │   ├── core/                     # コアロジック
│   │   │   ├── api/                  # APIクライアント (8ファイル)
│   │   │   ├── cache/                # キャッシュ (7ファイル) ★P1-3
│   │   │   ├── chat/                 # チャット処理
│   │   │   │   ├── classifier/       # Intent分類
│   │   │   │   ├── executors/        # Executor (5ファイル) ★P1-1
│   │   │   │   ├── apiExecutor.ts    # メインExecutor
│   │   │   │   └── intentClassifier.ts
│   │   │   ├── hooks/                # カスタムフック (1ファイル)
│   │   │   ├── refresh/              # リフレッシュ (3ファイル) ★P0-2
│   │   │   └── platform/             # プラットフォーム抽象化
│   │   ├── components/               # UIコンポーネント
│   │   ├── pages/                    # ページ
│   │   └── utils/                    # ユーティリティ
│   └── package.json
│
├── db/
│   └── migrations/                   # D1マイグレーション (62ファイル)
│       ├── 0001_init_core.sql
│       ├── ...
│       └── 0073_backfill_thread_timezone.sql
│
├── docs/                             # ドキュメント (120+ファイル)
│   ├── PROJECT_STATE_2026_01.md      # 完全状態ドキュメント ★NEW
│   ├── DISASTER_RECOVERY.md          # 復旧手順 ★NEW
│   ├── SOURCE_CODE_MAP.md            # このファイル ★NEW
│   ├── DATABASE_SCHEMA.md            # DB設計
│   ├── MIGRATION_HISTORY.md          # マイグレーション履歴
│   └── API_REFERENCE.md              # API仕様
│
└── package.json                      # ルートpackage.json
```

---

## 🔗 依存関係マップ

### Frontend コア依存関係

```
┌─────────────────────────────────────────────────────────────────┐
│                        UI Components                             │
│  (pages/*, components/*)                                        │
└─────────────────────┬───────────────────────────────────────────┘
                      │ import
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                     Custom Hooks                                 │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │ useViewerTimezone.ts                                      │  │
│  │   └── imports: getCachedMe, getMe, subscribeMe            │  │
│  │   └── imports: getBrowserTimeZone (utils/datetime)        │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────┬───────────────────────────────────────────┘
                      │ import
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                      Cache Layer                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ index.ts (exports all)                                      │ │
│  │   ├── meCache.ts         → usersMeApi                       │ │
│  │   ├── listsCache.ts      → listsApi                         │ │
│  │   ├── threadStatusCache.ts → threadsApi                     │ │
│  │   ├── threadsListCache.ts  → threadsApi                     │ │
│  │   ├── inboxCache.ts      → inboxApi                         │ │
│  │   └── useThreadStatus.ts → threadStatusCache                │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────┬───────────────────────────────────────────┘
                      │ import
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                       API Layer                                  │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ index.ts (exports all)                                      │ │
│  │   ├── client.ts          → fetch wrapper                    │ │
│  │   ├── usersMe.ts         → /api/users/me                    │ │
│  │   ├── lists.ts           → /api/lists                       │ │
│  │   ├── threads.ts         → /api/threads                     │ │
│  │   ├── contacts.ts        → /api/contacts                    │ │
│  │   ├── inbox.ts           → /api/inbox                       │ │
│  │   ├── calendar.ts        → /api/calendar                    │ │
│  │   ├── pendingActions.ts  → /api/pending-actions             │ │
│  │   └── voice.ts           → /api/voice                       │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
```

### Chat Executor 依存関係

```
┌─────────────────────────────────────────────────────────────────┐
│                     apiExecutor.ts (main)                        │
│                         63,950 lines                             │
└─────────────────────┬───────────────────────────────────────────┘
                      │ imports
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                      executors/                                  │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ index.ts                                                    │ │
│  │   ├── list.ts (8,367 lines)                                 │ │
│  │   │     └── executeListCreate, executeListList,             │ │
│  │   │         executeListMembers, executeListAddMember        │ │
│  │   │     └── imports: listsApi, contactsApi, refreshLists    │ │
│  │   │                                                         │ │
│  │   ├── thread.ts (14,935 lines)                              │ │
│  │   │     └── executeCreate, executeStatusCheck,              │ │
│  │   │         executeFinalize, executeThreadCreate,           │ │
│  │   │         executeInviteList                               │ │
│  │   │                                                         │ │
│  │   ├── calendar.ts (5,858 lines)                             │ │
│  │   │     └── executeToday, executeWeek, executeFreeBusy      │ │
│  │   │                                                         │ │
│  │   └── types.ts (5,662 lines)                                │ │
│  │         └── ExecutionResult, ExecutionContext               │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
```

### Refresh Layer 依存関係

```
┌─────────────────────────────────────────────────────────────────┐
│                      Executors                                   │
│  (executors/list.ts, executors/thread.ts, etc.)                 │
└─────────────────────┬───────────────────────────────────────────┘
                      │ calls runRefresh() or direct refresh
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                     runRefresh.ts                                │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ runRefresh(actions: RefreshAction[])                        │ │
│  │   ├── case 'STATUS':  → refreshStatus(threadId)             │ │
│  │   ├── case 'THREADS_LIST': → refreshThreadsListCache()      │ │
│  │   ├── case 'INBOX':   → refreshInboxCache()                 │ │
│  │   ├── case 'ME':      → refreshMeCache() ★P1-3              │ │
│  │   └── case 'LISTS':   → refreshListsCache() ★P1-3           │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────┬───────────────────────────────────────────┘
                      │ uses mapping from
                      ▼
┌─────────────────────────────────────────────────────────────────┐
│                     refreshMap.ts                                │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ WriteOp → RefreshAction[]                                   │ │
│  │                                                             │ │
│  │ THREAD_CREATE       → [STATUS, THREADS_LIST]                │ │
│  │ INVITE_SEND         → [STATUS, INBOX]                       │ │
│  │ ADD_SLOTS           → [STATUS]                              │ │
│  │ REMIND_PENDING      → [STATUS, INBOX]                       │ │
│  │ FINALIZE            → [STATUS, INBOX, THREADS_LIST]         │ │
│  │ USERS_ME_UPDATE_TZ  → [ME] ★P1-3                            │ │
│  │ LIST_CREATE         → [LISTS] ★P1-3                         │ │
│  │ LIST_ADD_MEMBER     → [LISTS] ★P1-3                         │ │
│  └────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────┘
```

---

## 📁 Backend Routes 詳細

| ファイル | バイト | 主要エンドポイント |
|---------|-------|------------------|
| `threads.ts` | 57,015 | /api/threads/* |
| `pendingActions.ts` | 24,739 | /api/pending-actions/* |
| `invite.ts` | 19,711 | /i/:token, /api/invites/* |
| `auth.ts` | 15,319 | /auth/* |
| `workItems.ts` | 13,114 | /api/work-items/* |
| `threadsFinalize.ts` | 12,998 | /api/threads/:id/finalize |
| `adminDashboard.ts` | 11,420 | /admin/* |
| `listItems.ts` | 11,322 | /api/list-items/* |
| `listMembers.ts` | 10,341 | /api/lists/:id/members/* |
| `lists.ts` | 10,090 | /api/lists/* |
| `businessCards.ts` | 9,733 | /api/business-cards/* |
| `threadsStatus.ts` | 9,726 | /api/threads/:id/status |
| `rooms.ts` | 8,009 | /api/rooms/* |
| `threadsRemind.ts` | 7,969 | /api/threads/:id/remind |
| `contacts.ts` | 7,699 | /api/contacts/* |
| `calendar.ts` | 7,605 | /api/calendar/* |
| `voice.ts` | 7,197 | /api/voice/* |
| `adminAi.ts` | 6,290 | /admin/ai/* |
| `inbox.ts` | 6,325 | /api/inbox/* |
| `usersMe.ts` | 3,907 | /api/users/me |
| `adminSystem.ts` | 3,684 | /admin/system/* |
| `otp.ts` | 3,929 | /api/otp/* |
| `testRateLimit.ts` | 1,253 | /test/rate-limit/* |
| `billing/index.ts` | - | /api/billing/* |
| `billing/me.ts` | - | /api/billing/me |
| `billing/myaspSync.ts` | - | /api/billing/myasp-sync |

---

## 📁 Frontend Cache 詳細

| ファイル | 行数 | 対象API | TTL |
|---------|------|---------|-----|
| `meCache.ts` | 231 | /api/users/me | 60s |
| `listsCache.ts` | 236 | /api/lists | 60s |
| `threadStatusCache.ts` | 303 | /api/threads/:id/status | 15s |
| `threadsListCache.ts` | 216 | /api/threads | 30s |
| `inboxCache.ts` | 216 | /api/inbox | 30s |
| `useThreadStatus.ts` | 197 | React hook | - |
| `index.ts` | 74 | exports | - |

---

## 📁 Migrations 詳細

### Phase別分類

| Phase | 範囲 | ファイル数 |
|-------|------|----------|
| 基盤 | 0001-0010 | 9 |
| 管理・システム | 0014-0018 | 5 |
| コミュニケーション | 0021-0031 | 10 |
| スケジュール調整 | 0032-0047 | 16 |
| 課金・リスト | 0048-0055 | 8 |
| ワークスペース | 0060-0064 | 5 |
| Beta A / Phase 2 | 0065-0073 | 9 |

### 重要マイグレーション

| 番号 | 名前 | 重要度 |
|------|------|-------|
| 0001 | init_core | ★★★ 基盤 |
| 0026 | threads_and_invites | ★★★ スレッド |
| 0034 | scheduling_slots | ★★★ スロット |
| 0065 | pending_actions | ★★ Beta A |
| 0066 | invite_deliveries | ★★ 配信追跡 |
| 0067-0069 | proposal_version | ★★ Phase 2 |
| 0072-0073 | timezone | ★★ TZ対応 |

---

## ✅ 検証済みファイル

### キャッシュ層（P1-3完了）

- [x] `meCache.ts` - TTL 60s, inflight共有, subscribe/unsubscribeログ
- [x] `listsCache.ts` - TTL 60s, inflight共有, subscribe/unsubscribeログ
- [x] `index.ts` - meCache/listsCache exports追加
- [x] `runRefresh.ts` - ME/LISTS refresh追加
- [x] `refreshMap.ts` - USERS_ME_UPDATE_TZ, LIST_CREATE, LIST_ADD_MEMBER追加

### Executor層（P1-1完了）

- [x] `list.ts` - refreshLists()呼び出し修正
- [x] `thread.ts` - 分割済み
- [x] `calendar.ts` - 分割済み

### Hooks（P1-3完了）

- [x] `useViewerTimezone.ts` - meCache連携

---

**作成**: 2026-01-17  
**更新**: 必要に応じて
